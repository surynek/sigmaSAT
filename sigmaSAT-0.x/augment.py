...
def print_header_info_frame_files(file, filename, width, height, version, product, copyright, author, email, url):
...
def augment_file(input_filename, output_filename, width, height, version, product, copyright, author, email, url):
  ...
  for line in input_file:
    if phase == 1:
      if string.find(line, "/") != 0:
        print_header_info_frame_files(output_file, input_filename, width, height, version, product, copyright, author, email, url)
...
def augment_directory(indent, directory, width, height, version, product, copyright, author, email, url):
  listing = os.listdir(directory)
  for item in listing:
    if os.path.isfile(item):
      if is_source(item):
        print indent + "Augmenting: " + item + " ... ",
        augment_file(item, item + ".aug", width, height, version, product, copyright, author, email, url)
      ...
      os.chdir(item)
      augment_directory(indent + "  ", ".", width, height, version, product, copyright, author, email, url)
      os.chdir("..")


def print_intro():
  print "Augment 1.0 - C++ Source Code Augmenter"
...
augment_directory("", ".", 80, 10, version, product, copyright, author, email, url)
